...
from hardware.airspynative import AirspyCallback, AirspyDevice, AirspyNativeError, parse_serial
from hardware.airspyprobe import apply_native_airspy_gain

logger = logging.getLogger("airspy-worker")
...
def _remove_dc_offset(samples: np.ndarray) -> np.ndarray:
    ...
    return samples - (mean_i + 1j * mean_q)


def airspy_worker_process(
    config_queue, data_queue, stop_event, iq_queue_fft=None, iq_queue_demod=None
):
    """
    Native libairspy worker process.

    It intentionally mirrors the queue metadata contract used by other SDR workers
    so the waterfall/demodulator pipeline does not need device-specific handling.
    """
    ...
    try:
        ...
        if not isinstance(config, dict):
            raise RuntimeError("Initial Airspy worker configuration is invalid")
        ...
        device = AirspyDevice.open(serial_value)
        ...
        device.set_sample_type_float32_iq()
        ...
        tuner_agc = bool(config.get("tuner_agc", False))
        bias_t = bool(config.get("bias_t", False))
        sdr_settings = config.get("sdr_settings") or {}
        if not isinstance(sdr_settings, dict):
            sdr_settings = {}
        apply_native_airspy_gain("airspy", device, gain, tuner_agc, bias_t, sdr_settings)

        logger.info(
            "Native Airspy configured: sample_rate=%s logical_center_freq=%s rf_center_freq=%s gain=%s",
            sample_rate,
            logical_center_freq,
            rf_center_freq,
            gain,
        ...
        def _airspy_callback(transfer_ptr):
            ...
            try:
                ...
                float_ptr = ctypes.cast(transfer.samples, ctypes.POINTER(ctypes.c_float))
                samples_view = np.ctypeslib.as_array(float_ptr, shape=(float_count,)).reshape(
                    sample_count, 2
                )
                complex_samples = (samples_view[:, 0] + 1j * samples_view[:, 1]).astype(
                    np.complex64, copy=False
                )
                sample_queue.put_nowait((complex_samples.copy(), int(transfer.dropped_samples)))
                ...
                if callback_error_logs < 3:
                    ...
                    logger.exception("Native Airspy callback conversion failed")
            return 0

        callback_ref = AirspyCallback(_airspy_callback)
        ...
        while not stop_event.is_set():
            ...
            while not config_queue.empty():
                ...
                if restart_stream:
                    ...
                    num_samples = calculate_samples_per_scan(sample_rate, fft_size)
                    pending_samples = np.empty(0, dtype=np.complex64)
                    logger.info("Updated Airspy sample rate: %s", sample_rate)
                ...
                if "bias_t" in new_config:
                    bias_t = bool(new_config.get("bias_t"))
                ...
                if (
                    ...
                    or "bias_t" in new_config
                    or "sdr_settings" in new_config
                ):
                    apply_native_airspy_gain(
                        "airspy", device, gain, tuner_agc, bias_t, sdr_settings
                    )
            ...
            try:
                ...
                if idle_for > 2.5 and (time.time() - last_callback_warn_at) > 5.0:
                    logger.warning(
                        "Native Airspy RX callback idle for %.2fs (cb=%s errors=%s drops=%s)",
                        idle_for,
            ...
            samples = require_complex64(samples, source="airspy-worker")
        ...
        message = f"Error in native Airspy worker process: {exc}"
        ...
        logger.info("Native Airspy worker process terminated")
